...
// const { userOne, admin, insertUsers, randomUser } = require('../fixtures/user.fixture');
...
//       const res = await request(app).post('/v1/auth/signup').send(newUser).expect(httpStatus.CREATED);
...
//         email: newUser.email,
//         role: 2,
//         // create_time: expect.any(Date), // THis could modified later
//         // update_time: expect.any(Date), // This could modified later
//         // boid: expect.anything(),
//       });
...
//       expect(dbUser).toMatchObject({ id: expect.anything(), email: newUser.email, role: 2 });
...
//       await request(app).post('/v1/auth/signup').send(newUser).expect(httpStatus.BAD_REQUEST);
//     });
...
//       const res = await request(app).post('/v1/auth/signin').send(signinCredentials).expect(httpStatus.OK);
...
//       const res = await request(app).post('/v1/auth/signin').send(signinCredentials).expect(httpStatus.BAD_REQUEST);
...
//       const res = await request(app).post('/v1/auth/signin').send(signinCredentials).expect(httpStatus.UNAUTHORIZED);
